...
class  AppFilterAdapter(mList: ArrayList<AppPackageModel>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        if (viewType == 0 || viewType == 2) {
            val v = LayoutInflater.from(parent.context).inflate(R.layout.dji_app_select_title_item, parent, false)
            return  AppFilterTitleHolder(v)
        }

        var view = LayoutInflater.from(parent.context).inflate(R.layout.dji_app_select_item, parent, false)
