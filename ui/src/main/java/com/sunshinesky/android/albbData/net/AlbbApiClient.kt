...
import com.sunshinesky.android.albbUtils.AlbbSSLSocketFactoryUtil
...
class AlbbApiClient private constructor() {
    ...
    lateinit var serviceAlbb: AlbbApiService

    private object Holder {
        val INSTANCE = AlbbApiClient()
    }
...
//                        else HttpLoggingInterceptor.Level.NONE
                ...
                .sslSocketFactory(AlbbSSLSocketFactoryUtil.createSSLSocketFactory())
                .addNetworkInterceptor(rewriteCacheControlInterceptor)
                .hostnameVerifier( AlbbSSLSocketFactoryUtil.TrustAllHostnameVerifier())
                .addInterceptor(baseInterceptor)
               // .addInterceptor(simulateResponseInterceptor)
                .addInterceptor(AlbbSignInterceptor())
                .build()

        val retrofit = Retrofit.Builder()
                .baseUrl(AlbbAppConfigData.runGateUrl)
                .addConverterFactory(GsonConverterFactory.create())
                .addCallAdapterFactory(RxJava2CallAdapterFactory.create())
                .client(mOkHttpClient)
                .build()

        serviceAlbb = retrofit.create(AlbbApiService::class.java)
    ...
    private var baseInterceptor: Interceptor = Interceptor { chain ->
        ...
        if (!AlbbNetWorkUtil.isNetWorkConnected()) {
            /**
            ...
            request = request.newBuilder()
                    ...
                    .build()
            AlbbLogUtils.i(TAG, "intercept:no network ")
        }
